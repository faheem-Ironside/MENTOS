...
def handle_at(bot, message):
    try:
        wait_message = bot.reply_to(message, "<b>Wait Checking...</b>", parse_mode="HTML")

        command_parts = message.text.split()
        if len(command_parts) < 2:
            bot.reply_to(message, "❌ *Usage:* `/at cardnumber|mm|yy|cvc`", parse_mode="Markdown")
            return

        card_details = command_parts[1].split("|")
        if len(card_details) != 4:
            bot.reply_to(message, "❌ *Invalid format!* Use `/at cardnumber|mm|yy|cvc`", parse_mode="Markdown")
            return
        ...
        lol = response_data.get("message", "Unknown error")

        if ("Your payment could not be taken." in lol or 
            "Cannot use a payment_method_nonce more than once." in lol or 
            "Merchant account does not support 3D Secure transactions for card type." in lol):
...
𝐁𝐚𝐧𝐤- {bank_name}
𝐂𝐨𝐮𝗻𝐭𝐫𝐲- {country_name} {country_flag}
